...
def get_scorecard_kpis(_conn, access_filter: str, year: int,
    ...
    sql = f"""
        SELECT
            ...
            COUNT(DISTINCT REFERENCE_TRIMMED_CUSTOMER_NAME) AS CUSTOMER_COUNT
        FROM {SCORECARD_TABLE}
        WHERE {where}
    """
    df = _run(_conn, sql)
...
def get_top_customers(_conn, access_filter: str, year: int,
    ...
    where = _build_scorecard_where(access_filter, years=[year],
                                   clients=list(clients) if clients else None)
    sql = f"""
        SELECT CUSTOMER_DISPLAY_NAME, DOLLARS, CASES, LBS, CLIENTS_SERVED
        FROM (
            SELECT
                CASE
                    WHEN REFERENCE_TRIMMED_CUSTOMER_NAME LIKE '%|%'
                    THEN TRIM(SPLIT_PART(REFERENCE_TRIMMED_CUSTOMER_NAME, '|', 1))
                    ELSE REFERENCE_TRIMMED_CUSTOMER_NAME
                END AS CUSTOMER_DISPLAY_NAME,
                SUM(DOLLARS) AS DOLLARS, SUM(CASES) AS CASES,
                SUM(LBS) AS LBS, COUNT(DISTINCT CLIENT_NAME) AS CLIENTS_SERVED
            FROM {SCORECARD_TABLE}
            WHERE {where}
              AND REFERENCE_TRIMMED_CUSTOMER_NAME IS NOT NULL
              AND TRIM(REFERENCE_TRIMMED_CUSTOMER_NAME) != ''
            GROUP BY CUSTOMER_DISPLAY_NAME
        )
...
def get_customer_churn(_conn, access_filter: str, current_year: int,
    ...
    sql = f"""
        WITH cy AS (
            SELECT DISTINCT REFERENCE_TRIMMED_CUSTOMER_NAME FROM {SCORECARD_TABLE}
            WHERE {where_cy} AND REFERENCE_TRIMMED_CUSTOMER_NAME IS NOT NULL AND TRIM(REFERENCE_TRIMMED_CUSTOMER_NAME) != ''
        ),
        py AS (
            SELECT DISTINCT REFERENCE_TRIMMED_CUSTOMER_NAME FROM {SCORECARD_TABLE}
            WHERE {where_py} AND REFERENCE_TRIMMED_CUSTOMER_NAME IS NOT NULL AND TRIM(REFERENCE_TRIMMED_CUSTOMER_NAME) != ''
        )
        SELECT
            (SELECT COUNT(*) FROM cy WHERE REFERENCE_TRIMMED_CUSTOMER_NAME NOT IN (SELECT REFERENCE_TRIMMED_CUSTOMER_NAME FROM py)) AS NEW_CUSTOMERS,
            (SELECT COUNT(*) FROM py WHERE REFERENCE_TRIMMED_CUSTOMER_NAME NOT IN (SELECT REFERENCE_TRIMMED_CUSTOMER_NAME FROM cy)) AS CHURNED_CUSTOMERS,
